...
class ListsSuite extends FunSuite {
  ...
  test("sum of a list of repeted elements should work") {
    assert(sum(List(1, 1, 1)) === 3)
  }
  
  test("sum with negative elements should work") {
    assert(sum(List(1, 5, -2)) === 4)
  }
  
  test("negative sum should be possible") {
  ...
  test("max of empty return an exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }
  
  test("max of a list of zero") {
    assert(max(List(0, 0, 0)) === 0)
  }
  
  test("max of a list of negative values") {
    assert(max(List(-2, -1, -5)) === -1)
  }
}
